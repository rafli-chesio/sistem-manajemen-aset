...
const props = defineProps({
    userRecord: { type: Object, required: true },
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h1 class="text-slate-800 font-bold text-lg">Edit Pengguna — {{ userRecord.name }}</h1>
        </template>

        <div class="max-w-xl mx-auto">
            <form @submit.prevent="form.post(route('users.update', userRecord.id))" class="space-y-5">
                <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Password Baru <span class="text-slate-400 text-xs">(kosongkan jika tidak diganti)</span></label>
